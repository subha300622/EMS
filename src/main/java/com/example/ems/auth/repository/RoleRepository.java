...
public interface RoleRepository extends JpaRepository<Role, Long> {
    @Query("SELECT r FROM Role r WHERE r.name = :name AND (r.isPlatformTemplate = true OR r.organization IS NULL)")
    ...
    @Query("SELECT r FROM Role r WHERE r.id = :id AND (r.organization.id = :organizationId OR (r.organization IS NULL AND r.isPlatformTemplate = true))")
    Optional<Role> findByIdAndOrganizationId(@Param("id") Long id, @Param("organizationId") Long organizationId);
}
