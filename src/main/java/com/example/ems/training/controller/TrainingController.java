...
public class TrainingController {
    ...
    @PutMapping("/trainings/courses/{id}")
    public ResponseEntity<?> updateCourse(
        ...
        Optional<TrainingCourseResponse> response = trainingService.updateCourse(id, request);
        if (response.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ErrorResponse.error("Training course not found with ID: " + id, "TRN_001"));
        }
        return ResponseEntity.ok(ApiResponse.success("Training course updated successfully", response.get()));
    }

    @DeleteMapping("/trainings/courses/{id}")
    public ResponseEntity<?> deleteCourse(
        ...
        boolean deleted = trainingService.deleteCourse(id);
        if (!deleted) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ErrorResponse.error("Training course not found with ID: " + id, "TRN_001"));
        }
        return ResponseEntity.ok(ApiResponse.success("Training course deleted successfully"));
    }
