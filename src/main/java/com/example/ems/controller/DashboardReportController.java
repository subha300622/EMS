...
public class DashboardReportController {
    ...
    public ResponseEntity<?> exportLeavesReport(
        ...
        StringBuilder csv = new StringBuilder("ID,Employee Name,Leave Type,Start Date,End Date,Status,Reason,Applied At\n");
        for (Leave l : list) {
    ...
    public ResponseEntity<?> exportEmployeesReport(
        ...
        StringBuilder csv = new StringBuilder("ID,Employee ID,Full Name,Email,Phone,Department,Designation,Status,Joining Date\n");
        for (Employee e : list) {
    ...
    public ResponseEntity<?> exportPayrollReport(
        ...
        if (!roleService.hasPermission(currentUser.getWorkEmail(), "reports.view")
                ...
                && !currentUser.getRole().getName().equals("SUPER_ADMIN")
                && !currentUser.getRole().getName().equals("ADMIN")
                && !currentUser.getRole().getName().equals("FINANCE")) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
