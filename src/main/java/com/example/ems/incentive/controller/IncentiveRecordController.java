...
public class IncentiveRecordController {
    ...
    public ResponseEntity<ApiResponse<IncentiveRecordResponse>> getIncentiveById(@PathVariable("id") Long id) {
        IncentiveRecordResponse response = workflowService.getRecordById(id);
        return ResponseEntity.ok(ApiResponse.success("Incentive record retrieved successfully", response));
    }
    ...
    public ResponseEntity<ApiResponse<Page<IncentiveRecordResponse>>> getIncentiveRecords(
        ...
        Page<IncentiveRecordResponse> records = workflowService.searchRecords(employeeId, status, payrollStatus, periodStart, periodEnd, pageable);
        return ResponseEntity.ok(ApiResponse.success("Incentive records retrieved successfully", records));
    }
    ...
    public ResponseEntity<ApiResponse<Page<IncentiveRecordResponse>>> getMyIncentiveRecords(
            ...
            @RequestParam(defaultValue = "20") int size) {
        Pageable pageable = PageRequest.of(Math.max(0, page), Math.min(100, Math.max(1, size)), Sort.by(Sort.Direction.DESC, "createdAt"));
        Page<IncentiveRecordResponse> records = workflowService.getMyRecords(pageable);
        return ResponseEntity.ok(ApiResponse.success("My incentive records retrieved successfully", records));
    }
}
