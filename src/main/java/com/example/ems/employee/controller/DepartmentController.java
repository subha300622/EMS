    ...
    public ResponseEntity<?> getEmployeesByDepartment(
        ...
        return departmentService.getDepartmentById(id)
                .<ResponseEntity<?>>map(dept -> {
                    java.util.List<com.example.ems.employee.entity.Employee> employees = employeeService.getEmployeesByDepartment(dept.getName());
                    return ResponseEntity.ok(ApiResponse.success("Employees retrieved successfully for department: " + dept.getName(), employees));
                })
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ErrorResponse.error("Department not found with ID: " + id, "DEP_001")));
    }
