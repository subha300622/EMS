...
public class SaaSOrganizationPaymentFlowIntegrationTest {
        ...
        public void setUp() {
                mockMvc = MockMvcBuilders
                                .standaloneSetup(organizationController, subscriptionController, paymentController)
                ...
                token = "Bearer " + jwtService.generateAccessToken(adminUser.getUserId(), adminUser.getWorkEmail(),
        ...
        public void testSaaSOrganizationPaymentFlow() throws Exception {
                ...
                orgRequest.setAddress(
                                new OrganizationAddressDto("123 Flow St", "Tech City", "Karnataka", "India", "560001"));
                ...
                System.out.println(mapper.writerWithDefaultPrettyPrinter()
                                .writeValueAsString(mapper.readTree(orgRespJson)));

                com.fasterxml.jackson.databind.JsonNode orgNode = mapper.readTree(orgRespJson);
                long orgId = orgNode.get("data").get("id").asLong();

                // Clear default active subscription created automatically, so we can test
                ...
                BillingDto billingDto = new BillingDto("YEARLY", BigDecimal.valueOf(50000.00), "INR",
                                BigDecimal.valueOf(9000.00), BigDecimal.valueOf(2000.00), BigDecimal.valueOf(57000.00));
                ...
                FeaturesDto featuresDto = new FeaturesDto(true, true, true, true, true, true, true, true, true, true,
                                true, true, true);
                ...
                System.out.println(mapper.writerWithDefaultPrettyPrinter()
                                .writeValueAsString(mapper.readTree(subRespJson)));
                ...
                System.out.println(mapper.writerWithDefaultPrettyPrinter()
                                .writeValueAsString(mapper.readTree(orderRespJson)));
                ...
                // Step 5: Gateway Webhook Trigger (Sig verification bypass via standalone
                // header validation)
                ...
                System.out.println(mapper.writerWithDefaultPrettyPrinter()
                                .writeValueAsString(mapper.readTree(webhookPayload)));
                ...
                // 1. GET /overview
                MvcResult overviewResult = mockMvc
                                .perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                                .get("/api/v1/platform-admin/subscriptions/overview")
                                                .header("Authorization", token))
                                .andExpect(status().isOk())
                                .andReturn();
                ...
                System.out.println(mapper.writerWithDefaultPrettyPrinter()
                                .writeValueAsString(mapper.readTree(overviewJson)));

                // 2. GET /metrics/mrr
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/mrr")
                                .header("Authorization", token))
                                .andExpect(status().isOk());

                // 3. GET /metrics/active
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/active")
                                .header("Authorization", token))
                                .andExpect(status().isOk());

                // 4. GET /metrics/revenue
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/revenue")
                                .header("Authorization", token))
                                .andExpect(status().isOk());

                // 5. GET /metrics/invoices
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/invoices")
                                .header("Authorization", token))
                                .andExpect(status().isOk());

                // 6. GET /metrics/plan-distribution
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/plan-distribution")
                                .header("Authorization", token))
                                .andExpect(status().isOk());

                // 7. GET /metrics/renewals
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/renewals?days=30")
                                .header("Authorization", token))
                                .andExpect(status().isOk());

                // 8. GET /metrics/churn
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/api/v1/platform-admin/subscriptions/metrics/churn")
                                .header("Authorization", token))
                                .andExpect(status().isOk());
