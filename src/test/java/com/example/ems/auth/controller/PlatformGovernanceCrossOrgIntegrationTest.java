...
public class PlatformGovernanceCrossOrgIntegrationTest {
    ...
    @Autowired
    private PlatformOrganizationRoleController platformOrganizationRoleController;
    ...
    public void setUp() {
        ...
        platformRoleMvc = MockMvcBuilders.standaloneSetup(platformOrganizationRoleController).build();
        ...
        platformAdminToken = "Bearer " + jwtService.generateAccessToken(
                platformAdmin.getUserId(), platformAdmin.getWorkEmail(), "SUPER_ADMIN", null, null, 1, 0
        );
    ...
    public void testPlatformOrganizationUserViewAndOverride() throws Exception {
        ...
        platformUserMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/users/" + regularUser.getId())
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        platformUserMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/users/" + regularUser.getId() + "/permissions")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data").isArray());
        ...
        platformUserMvc.perform(put("/api/v1/platform/organizations/" + testOrg.getId() + "/users/" + regularUser.getId() + "/role")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(reqNoReason)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Reason is required for auditing platform admin overrides."));
    ...
    @Test
    public void testPlatformOrganizationRoleManagementAndOverride() throws Exception {
        // Create custom role
        RoleRequest newRoleReq = new RoleRequest();
        newRoleReq.setName("GOV_LEAD");
        newRoleReq.setDescription("Governance Lead Custom Role");
        String createdResponse = platformRoleMvc.perform(post("/api/v1/platform/organizations/" + testOrg.getId() + "/roles")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(newRoleReq)))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();

        Role createdRole = objectMapper.readValue(
                objectMapper.readTree(createdResponse).get("data").toString(),
                Role.class
        );
        assertNotNull(createdRole);
        assertEquals("GOV_LEAD", createdRole.getName());

        // List custom roles
        platformRoleMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/roles")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));

        // Get role users (empty)
        platformRoleMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/roles/" + createdRole.getId() + "/users")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").isArray())
                .andExpect(jsonPath("$.data").isEmpty());

        // Override role permissions with reason
        Permission p = permissionRepository.save(new Permission(null, "platform.test.perm", "Test Platform Permission"));
        OverrideRequest overrideReq = new OverrideRequest(List.of(p.getId()), "Compliance requirements alignment");
        platformRoleMvc.perform(put("/api/v1/platform/organizations/" + testOrg.getId() + "/roles/" + createdRole.getId() + "/permissions")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken)
                .contentType(MediaType.APPLICATION_JSON)
        ...
        // Template comparison and synchronization preview
        Role dbAdminRole = roleRepository.findByOrganizationIdAndName(testOrg.getId(), "ADMIN").orElseThrow();
        platformRoleMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/roles/" + dbAdminRole.getId() + "/template-diff")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.added").isArray());

        // Role usage stats
        platformRoleMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/roles/role-stats")
                .header(HttpHeaders.AUTHORIZATION, platformAdminToken))
                .andExpect(status().isOk())
    ...
    public void testUserRoleControllerEndpoints() throws Exception {
        ...
        // Assign role to user
        Role tenantAdminRole = roleRepository.findByOrganizationIdAndName(testOrg.getId(), "ADMIN").orElseThrow();
    ...
    public void testPermissionsLeastPrivilegeAccessDenied() throws Exception {
        ...
        dashboardMvc.perform(get("/api/v1/platform/dashboard")
                .header(HttpHeaders.AUTHORIZATION, orgAdminToken))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access Denied: Requires 'platform.dashboard.view' permission."));

        // 2. Organization List: non-admin gets 403 Forbidden
        platformOrgMvc.perform(get("/api/v1/platform/organizations")
                .header(HttpHeaders.AUTHORIZATION, orgAdminToken))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access Denied: Requires 'platform.organization.view' permission."));

        // 3. Audit Logs: lack of 'platform.audit.view' gets 403 Forbidden
        platformOrgMvc.perform(get("/api/v1/platform/organizations/" + testOrg.getId() + "/audit-logs")
                .header(HttpHeaders.AUTHORIZATION, orgAdminToken))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access Denied: Requires 'platform.audit.view' permission."));
        ...
        platformUserMvc.perform(put("/api/v1/platform/organizations/" + testOrg.getId() + "/users/" + regularUser.getId() + "/role")
                ...
                .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access Denied: Requires 'platform.role.override' permission."));

        // 5. Override Role Permissions: lack of 'platform.permission.override' gets 403 Forbidden
        Role dbAdminRole = roleRepository.findByOrganizationIdAndName(testOrg.getId(), "ADMIN").orElseThrow();
        OverrideRequest overrideReq = new OverrideRequest(List.of(1L), "Reason test");
        platformRoleMvc.perform(put("/api/v1/platform/organizations/" + testOrg.getId() + "/roles/" + dbAdminRole.getId() + "/permissions")
                .header(HttpHeaders.AUTHORIZATION, orgAdminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(overrideReq)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access Denied: Requires 'platform.permission.override' permission."));
    }
}
